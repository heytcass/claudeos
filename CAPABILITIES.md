...
ClaudeOS is a Claude-native NixOS system where Claude is integrated at every layer: desktop keybindings, shell commands, MCP servers, Claude Code agents/skills/hooks, and background monitoring. This document is the single reference for what's available and when to use it.
...
Hooks fire automatically on certain events — no manual invocation needed.

| Hook | Trigger | What it does |
|------|---------|-------------|
| **SessionStart** | Every new Claude Code session | Injects current hostname, available hosts, and build/deploy commands into context |
| **PostToolUse** (Edit/Write) | After any `.nix` file is edited | Auto-formats with `nixfmt` |
| **PreToolUse** (Edit/Write) | Before editing `hardware-configuration.nix` | **Blocks the edit** — this file is auto-generated by NixOS |
...
Fish shell functions available in any terminal. Defined in `home/shell/fish.nix`.
...
| **Jasper** | Always running | AI companion daemon in the Noctalia system bar. Has its own Anthropic API key, Google integrations for weather/calendar. |
