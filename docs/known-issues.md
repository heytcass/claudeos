...
- 2026-08-25 · Fresh boot came up with no bar, no cursor theme, no keyring, and Hyprland's default wallpaper; `hyprctl configerrors` empty, 54 binds registered, no Lua error anywhere in the log · **RESOLVED same day by rollback to generation 41 + dropping the feature. Root cause: a `set_workspace` call at CONFIG-LOAD time aborted the IIFE that was supposed to register the `hyprland.start` hook.** Second-desk work bound workspaces per-desk by registering `hl.workspace_rule` objects inside an IIFE whose return value was the start callback: `hl.on("hyprland.start", (function() … return function() … end end)())`. The IIFE ended with `apply_desk_workspaces()`, which calls `monitor:set_workspace({workspace=N})`. At a **real boot** monitors exist but workspaces do not yet, so that call throws, the IIFE never reaches its `return`, and `hl.on` is never called — silently costing every `exec_cmd` in the start hook (`qs`, `hyprctl setcursor`, `gnome-keyring-daemon`). Diagnostic signature worth memorizing: **Hyprland executes top-level config statements independently**, so everything before the throw (the four rule objects, all four `set_enabled` calls) AND everything after the failed statement (window rules, `settings.workspace_rule`, all binds) applied normally — the *only* casualty was the lost subscription, which produces no error output at all. Confirm with `pgrep qs` plus `grep exec <instance>/hyprland.log`; an empty grep means the start hook never ran. **Process failure that let it ship: every validation was `hyprctl reload`, where workspaces already exist (so the call succeeds) and the bar is already running (so a dead start hook is invisible). `Hyprland --verify-config` cannot catch it either — it checks that the config parses, and with no monitors present the risky branch is never entered.** Anything touching the start hook or compositor state at load time MUST be tested by an actual reboot. Rule of thumb for a rebuild: register `hl.on(...)` FIRST, before any call that can throw, so a bug in a feature can never take the bar down with it. Monitor *geometry* for the second desk (`desc:…P2425HE J8CTK14`, 1920x1080@100 at 3840x921) was kept — it is declarative `hl.monitor` with no hooks and was never implicated.

- 2026-08-25 · regreet/Quickshell greeter draws its login box straddling two monitors (text field split down the bezel) rather than centring on one · Reproduced on generation 41 (Aug 19 inputs, none of the second-desk changes), so it is **pre-existing and independent of both the desk work and the flake-input bump** — it simply had not been hit before at a desk with this geometry. Untriaged. Next step: check whether the greeter QML anchors to a specific screen or to the whole logical layout (`modules/desktop/greeter.nix` + the shared Theme singleton); likely needs a per-screen `Variants`/screen binding like the bar uses.

- 2026-08-20 · Screen found dark and unwakeable after 4 days away — mouse/keyboard did nothing, `hyprctl dispatch dpms on` failed with exit 7 (`')' expected near 'on'`) · **RESOLVED same day — two stacked casualties of the 2026-08-15 Lua config migration, the second one a silent-toggle trap worth memorizing.** (1) Under `CONFIG_LUA`, `hyprctl dispatch` wraps its argument verbatim in `hl.dispatch(...)`, so every legacy-form dispatch in hypridle's config (`dpms off` at 10 min, `dpms on` on resume, `after_sleep_cmd`) had been failing since the migration; with `misc:mouse_move_enables_dpms` / `key_press_enables_dpms` both defaulting to false, once the screen blanked *nothing* could wake it — same casualty class as the `hyprctl keyword` breakage in the 08-15 entry below. (2) The obvious Lua rewrite is a trap: `hl.dsp.dpms("on")` and `hl.dsp.dpms(true)` parse and dispatch fine but a **non-table argument silently means TOGGLE** (0.56.2 `LuaBindingsInternal.cpp:tableToggleAction` — `if (!lua_istable(...)) return TOGGLE_ACTION_TOGGLE`), producing an "on" that turns the screen off every other call; `parseToggleStr` also maps *unknown strings* to toggle, so a typo'd action toggles too. Correct form: `hl.dsp.dpms({action="on"|"off"})` (also accepts `enable`/`disable`; optional `monitor` field), executed imperatively as `hyprctl dispatch 'hl.dsp.dpms({action="on"})'`. Setter semantics proven live (on,on holds; off drops; on restores; exact sh-quoted strings verified against `hyprctl -j monitors` dpmsStatus). Fixed in home/hyprland.nix (hypridle listeners + after_sleep_cmd). Debugging lesson recorded with intent: `hyprctl eval` prints `ok` and DISCARDS return values and constructed-but-undispatched objects — three separate false "it worked" readings in one session came from trusting `ok`; verify dpms against `hyprctl -j monitors`, never against eval output. Related audit: the config's own bind tables consume dispatcher *objects* (`hl.dsp.exec_cmd(...)` etc.), which is the intended use — only imperative `hl.dispatch(obj)` call sites need the table-arg discipline, and the repo's only togglable-dispatcher user is dpms.

- 2026-08-17 · `Failed to start home-ops infrastructure health check` (1 occurrence) · **RESOLVED 2026-08-19 — three designs interacting, each fine alone; diagnosed and fixed same day.** `home-ops-health` exits 1 on findings BY DESIGN and, as a weekly oneshot, latches `failed` for up to a week (hosts/gti/default.nix: "THE FAILED UNIT IS THE SIGNAL"). `claudeos-health-check`'s 15-min failed-unit sweep re-read that latch as fresh every run (283 consecutive failures 08-16→19), and `claudeos-notify`'s cooldown was time-only with no memory of content — the same fact re-worded through haiku and re-bannered every 30 min, ~48/day. Fixed in a687a22 (content-fingerprint dedup: new content notifies within the 30-min churn floor, unchanged content reminds once per 24h — staged, active after next reboot). The stale latch was reset 2026-08-19 (`systemctl --user reset-failed home-ops-health`); it re-latches at the next Sunday run if findings persist, which is the designed behavior. The underlying findings belong to the home-ops repo and were triaged 08-19: HA core restarted 08-18 13:39 and the unavailable-entity jump (57→87→154) is integrations/devices that did not come back — 49 = `vaca` (Primary Bedroom Display) stuck in setup_retry, 31 = proxmoxve control buttons (hypervisor itself healthy, VM uptime sensors live), one dead Zigbee temp sensor (`living_room_mho_c401n_z`, likely battery), remainder expected-off devices (kiosks, TVs, satellites). Nothing security-relevant: cameras live, no locks/doors affected.

- 2026-08-16 · oo7-daemon.service: Failed at step EXEC spawning /run/wrappers/bin/oo7-daemon: No such file or directory (5 occurrences) · **RESOLVED 2026-08-19 — no action; artifact of the reverted oo7 migration.** These were the death throes of the migration rolled back 2026-08-15 ~22:44 (postmortem: the 2026-08-15 oo7 entries below, merged as PR #106): the unit spawned `/run/wrappers/bin/oo7-daemon`, a path only the migration generation provided, so activation transitions around the revert failed EXEC against the restored generation. The unit does not exist on the current generation (`systemctl --user cat oo7-daemon` → not found); its last journal activity is the 22:44 stop. The diary's suggested next step ("verify oo7-daemon package is declared") is answered: it is not, deliberately — see the do-not-retry verdict in the postmortem below.
